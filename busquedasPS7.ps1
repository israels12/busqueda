...
if (-not (Test-Path $sevenZipPath)) {
    ...
    if (Test-Path $sevenZipPath) {
        ...
        Write-Host "La instalación de 7-Zip ha fallado."
        exit
    }
}
...
if ($decompress -eq 's') {
    $pattern = Read-Host "Ingrese el patrón de archivo (por ejemplo, *.rar, *.zip)"
    ...
    foreach ($file in $files) {
        ...
        if ($extension -eq '.zip') {
            ...
            Write-Host "Formato de archivo no soportado. Sólo se soportan .rar y .zip." -ForegroundColor Red
        }
    }
...
foreach ($result in $results) {
    ...
    foreach ($pattern in $patterns) {
        $line = $line -replace $pattern, "`e[31m$pattern`e[0m"  # `e[31m` es el código ANSI para el color rojo y `e[0m` para resetear el color
    }
